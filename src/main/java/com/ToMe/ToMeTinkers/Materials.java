...
public class Materials {
	...
	// if(TinkerRegistry.getMaterial(name) == Material.UNKNOWN &&
	// TConstruct.pulseManager.isPulseLoaded(TinkerSmeltery.PulseId)) {
	// if(TinkerRegistry.getMaterial(name) == Material.UNKNOWN &&
	// isSmelteryEnabled()) {
	...
	// ToMeTinkers.logger.info("" +
	// FluidRegistry.getFluid("tconstruct:molten_glass"));
	...
	// glassIntegration = new MaterialIntegration(glass,
	// FluidRegistry.getFluid("tconstruct:molten_glass"), "Glass");
	// glassIntegration = new MaterialIntegration(glass,
	// FluidRegistry.getFluid("tconstruct:glass"), "Glass");
	// glassIntegration = new MaterialIntegration(glass,
	// FluidRegistry.getFluid("glass"), "Glass");
	...
	public static void registerMaterials() {
		...
		// TinkerRegistry.registerMelting(new
		// MeltingRecipe(RecipeMatch.of(Items.GLASS_BOTTLE, 1000),
		// FluidRegistry.getFluid("glass")));
		// TinkerRegistry.registerMelting(Items.GLASS_BOTTLE,
		// FluidRegistry.getFluid("glass"), 1000);
		// TinkerRegistry.registerMelting(Items.GLASS_BOTTLE, glassFluid, 1000);
		// TinkerRegistry.addMaterialStats(glass, glassHead, glassHandle, glassExtra);
		// TinkerRegistry.addMaterialStats(glass, glassHead, glassHandle, glassExtra,
		// glassBow);
		...
		for (MaterialStats stats : MaterialStats.values()) {
			...
			// if(TinkerRegistry.getMaterial(stats.id) == Material.UNKNOWN &&
			// ToMeTinkers.cfg.getMaterialEnable(stats.id)) {
			// stats.addItems();
			// stats.addTraits();
			// stats.register();
			// }
	...
	public static void registerFluids(RegistryEvent.Register<Block> e) {
		...
		for (FluidMolten fm : MaterialStats.fluids) {
			...
			// registry.register(new BlockMolten(fm).setUnlocalizedName(ToMeTinkers.MODID +
			// ":molten_" + fm.getName()).setRegistryName(ToMeTinkers.MODID + ":molten_" +
			// fm.getName()/*new ResourceLocation(ToMeTinkers.MODID, fm.getName())*/));
			// registry.register(new BlockMolten(fm).setUnlocalizedName(ToMeTinkers.MODID +
			// ".molten_" + fm.getName()).setRegistryName(ToMeTinkers.MODID + ".molten_" +
			// fm.getName()));
			// registry.register(new BlockMolten(fm).setUnlocalizedName(ToMeTinkers.MODID +
			// ".molten_" + fm.getName()).setRegistryName("molten_" + fm.getName()));
			registry.register(new BlockMolten(fm).setUnlocalizedName(ToMeTinkers.MODID + ".molten_" + fm.getName())
					.setRegistryName(new ResourceLocation(ToMeTinkers.MODID, "molten_" + fm.getName())));
			// registry.register(new BlockMolten(fm).setUnlocalizedName(ToMeTinkers.MODID +
			// ".molten_" + fm.getName()).setRegistryName(new ResourceLocation("tconstruct",
			// "molten_" + fm.getName())));
			if (Config.debug) {
